class ArticleImage
  ...
  def initialize(image_path)
    ...
    path = File.basename(image_path, '.*')[2..-1]
    @order = path.split('-')[0].to_i
    @title = path.split('-')[1..-1].join(' ').gsub('_', ' ').capitalize
  end
  ...
  def self.load(params = {})
    raise 'You must provide a ":loadpath" parameter' unless params[:load_path]
    raise 'You must provide a ":article_id" parameter' unless params[:article_id]
